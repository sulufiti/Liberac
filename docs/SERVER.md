...
There's a git repository which just consists of hooks that run whenever updates are received. You, the developer, push your changes to the git repo which updates, runs the hooks (which updates ~/liberac) and then exits.

The actual `liberac` folder however isn't linked to the repo at all. It's just a copy of whatever you push to the server via git.

The Node application itself is run by [pm2](https://github.com/Unitech/pm2) which watches for folder changes and upon so, restarts itself to reflect the latest changes. It's important to check that `watch` is enabled as manually restarts usually disable it. As pm2 serves the application from memory (as far as I understand), if watch is disabled, it won't automatically update anymore.

The pm2 configurations are stored in the repo as `ecosystem.config.js`. If you ever delete the environments, you just need to run `pm2 start ecosytem.config.js` to recreate them.

## VM layout

The location of everything is as follows:

| Item | Location | What it does |
| ---- | -------- | ------------ |
| apache2 | /etc/apache2 | Serves the Node application (or whatever happens to be hosted on Port 3000 internally) |
| backups | ~/backups | Nightly backups of the Liberac production database as importable files |
| deploy | ~/deploy | The repo which `git push production master` talks to. Hooks are stored in, you guessed it, `~/deploy/hooks` |
| liberac | ~/liberac | Server copy of the application (generated by deploy) |
| scripts | ~/scripts| A copy of the bash script which generates backups. Run nightly by `crontab` |
| azurestorage | /mnt/azurestorage | Azure File Storage mounted as a drive. Backups are copied here as well to the `postgres` folder. This will likely be dismounted upon restart so you'll likely need to manually remount it. There's likely/definitely a better way of doing this too |
